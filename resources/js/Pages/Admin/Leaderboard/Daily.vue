...
const exportToPdf = () => {
    ...
    if (sortKey.value) params.append('sort_key', sortKey.value);
    if (sortDirection.value) params.append('sort_dir', sortDirection.value);
    window.open(route('admin.leaderboard.daily.pdf') + '?' + params.toString(), '_blank');
};
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                ...
                <div class="flex flex-wrap gap-2">
                    <button
                        @click="exportToPdf"
                        class="flex items-center justify-center gap-2 px-4 py-2 bg-rose-700 hover:bg-rose-600 text-white font-semibold text-sm rounded-lg transition-colors duration-200 shadow-lg shadow-rose-950/30"
                    >
