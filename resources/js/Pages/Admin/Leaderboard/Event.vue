...
const resetFilters = () => {
    if (props.eventQuizzes && props.eventQuizzes.length > 0) {
        eventId.value = props.eventQuizzes[0].id;
    } else {
        eventId.value = '';
    }
    searchQuery.value = '';
...
const exportToExcel = () => {
    ...
    window.location.href = route('admin.leaderboard.event.export') + '?' + params.toString();
};
...
<template>
    <Head title="Leaderboard Kuis Event" />

    <AdminDashboardLayout>
        <template #header>
            <span>Leaderboard Kuis Event</span>
        </template>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                    <div class="w-full sm:w-64">
                        <select
                            v-model="eventId"
                            class="w-full bg-slate-950 border border-slate-850 text-slate-300 text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-slate-700 transition-colors duration-200"
                        >
                            <option value="" disabled>Pilih Kuis Event</option>
                            <option v-for="quiz in eventQuizzes" :key="quiz.id" :value="quiz.id">
                                {{ quiz.title }}
                            </option>
                        </select>
                    </div>
                    <button
                        @click="resetFilters"
                        class="px-4 py-2.5 w-full sm:w-auto bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold text-sm rounded-xl transition-colors duration-200"
                    >
                        Reset Filter
                    </button>
                </div>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-3">
                    <h3 class="font-bold text-slate-200">Leaderboard Kuis Event Petugas</h3>
                    <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                        <select
                        ...
                        <tbody class="divide-y divide-slate-800 text-sm">
                            ...
                            <tr v-if="leaderboard.data.length === 0">
                                <td colspan="10" class="py-8 text-center text-slate-500 text-xs">
                                    Tidak ada data kuis untuk pencarian atau event ini.
                                </td>
                            </tr>
                        </tbody>
                    </table>
