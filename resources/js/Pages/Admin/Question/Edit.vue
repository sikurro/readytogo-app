...
const form = useForm({
    ...
    answers: props.question.answers.map(a => ({
        ...
        answer_image: null,
        remove_answer_image: false,
        is_correct: a.is_correct === 1 || a.is_correct === true || a.is_correct === '1',
    })),
});
...
const handleAnswerImageChange = (e, index) => {
    ...
    if (file) {
        ...
        form.answers[index].remove_answer_image = false;
    }
};
...
const removeQuestionImage = () => {
    ...
    form.remove_question_image = true;
...
const removeAnswerImage = (index) => {
    ...
    form.answers[index].remove_answer_image = true;
...
const submitForm = () => {
    form
        .transform((data) => {
            const cleanedAnswers = data.answers.map(ans => {
                const copy = { ...ans };
...
<template>
    <Head title="Edit Soal - Admin" />

    <AdminDashboardLayout>
        <template #header>
            <span>Edit Soal Master Bank</span>
        </template>
        ...
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <form @submit.prevent="submitForm" class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
                ...
                <div class="border-b border-slate-800 pb-4">
                    <h3 class="text-lg font-bold text-slate-200">Formulir Edit Soal</h3>
                    <p class="text-sm text-slate-400">Silakan perbarui detail pertanyaan, kategori, opsi jawaban, dan kunci jawaban di bawah.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-slate-950/40 rounded-xl border" :class="form.errors.question_image ? 'border-rose-500/50' : 'border-slate-800'">
                    ...
                    <div class="relative flex flex-col justify-center items-center bg-slate-950/80 rounded-lg p-2 min-h-[120px] border border-slate-800/50">
                        <button 
                            v-if="questionImagePreview || (question.question_image && !form.remove_question_image)" 
                            type="button" 
                        ...
                        <img v-else-if="question.question_image && !form.remove_question_image" :src="question.question_image" class="max-h-24 object-contain rounded" />
                ...
                <div class="space-y-4 pt-4 border-t border-slate-800">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs text-slate-400 italic">*Pilih satu jawaban yang benar menggunakan tombol radio</span>
                    </div>
                                ...
                                <div class="flex-1 space-y-3">
                                    <div>
                                        <input 
                                            ...
                                            :required="!ans.answer_image && (!question.answers[index]?.answer_image || ans.remove_answer_image)"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center bg-slate-950/40 p-3 rounded-lg border border-slate-800/50">
                                        ...
                                        <div class="relative flex justify-center items-center bg-slate-950/80 rounded p-1 min-h-[50px] border border-slate-800/30">
                                            <button 
                                                v-if="answerImagePreviews[index] || (question.answers[index] && question.answers[index].answer_image && !ans.remove_answer_image)" 
                                                type="button" 
                                            ...
                                            <!-- Existing image preview -->
                                            <img v-else-if="question.answers[index] && question.answers[index].answer_image && !ans.remove_answer_image" :src="question.answers[index].answer_image" class="max-h-12 object-contain rounded" />
                ...
                <div class="bg-slate-800/30 -mx-6 -mb-6 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-800 rounded-b-2xl">
                    <button 
                        ...
                        class="inline-flex justify-center rounded-lg border border-transparent px-5 py-2.5 bg-amber-600 text-sm font-semibold text-white hover:bg-amber-500 focus:outline-none transition-colors disabled:opacity-50 shadow-lg shadow-amber-900/20"
                    >
                        Simpan Perubahan
                    </button>
                    <Link 
